...
function PracticeTest({ questions, userAnswers, onAnswerSelect, currentQuestionIndex, onSetCurrentQuestionIndex, onSubmitTest }) {
  // Defensive check to ensure questions array is valid before accessing elements
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="bg-white p-8 rounded-xl shadow-2xl max-w-2xl w-full mt-8 mb-8">
        <h2 className="text-3xl font-extrabold text-gray-800 mb-6 text-center font-inter">Practice Test</h2>
        {/* The loading check above handles the questions.length === 0 case */}
        <>
          ...
          <div className="flex justify-center flex-wrap gap-1 mb-4 overflow-x-auto whitespace-nowrap py-2 no-scrollbar">
            {questions.map((q, index) => (
              <button
                key={index}
                onClick={() => onSetCurrentQuestionIndex(index)}
                // Disable if not answered AND not the current question
                disabled={userAnswers[q.id] === undefined && index !== currentQuestionIndex}
                className={`w-6 h-6 text-sm rounded-full flex-shrink-0 flex items-center justify-center font-bold transition-colors duration-200
                  ...
                  ${userAnswers[q.id] === undefined && index !== currentQuestionIndex ? 'opacity-50 cursor-not-allowed' : ''}
                  `}
          ...
          {/* End Optimized Navigation Section */}
